...
def compact_payload_for_remote(workflow: str, payload: AiWorkflowIn) -> dict[str, Any]:
    ...
    if workflow in {
        ...
        "revise_selection",
        "generate_emotion_seed",
        "emotion_archaeology",
        "dialogue_subtext_excavation",
        "analyze_reader_pull",
        "deepen_and_bury",
        "anti_ai_polish",
    }:
        ...
        data["content"] = trim_text(data.get("content", ""), 3500)
...
def emotional_source_text(payload: AiWorkflowIn, context: dict[str, Any]) -> str:
    extra = getattr(payload, "model_extra", {}) or {}
    for key in ("selected_text", "content", "current_draft"):
        value = extra.get(key) if key in extra else getattr(payload, key, "")
        if isinstance(value, str) and value.strip():
            return trim_text(value.strip(), 900)
    chapter = context.get("chapter") if isinstance(context.get("chapter"), dict) else {}
    return trim_text(str(chapter.get("draft") or chapter.get("brief") or payload.prompt or ""), 900)


def build_emotional_rewrite(payload: AiWorkflowIn, context: dict[str, Any], anti_ai: bool = False) -> str:
    source = emotional_source_text(payload, context)
    protagonist = primary_character_name(context)
    if not source:
        source = f"{protagonist}停在门口，没有马上回答。"
    sensory_anchor = "雨水、旧纸气味和指尖停顿"
    if anti_ai:
        return (
            source.rstrip("。")
            + f"。{protagonist}没有把那句话说完，只把视线移到窗缝里渗进来的水痕上。"
            "那里没有答案，却比任何解释都诚实。"
        )
    return (
        source.rstrip("。")
        + f"。这一次，{sensory_anchor}替人物把没说出口的部分留在了场面里；"
        f"{protagonist}真正害怕的不是失败，而是自己已经习惯把失去说成选择。"
    )
...
def structured_output_for_workflow(workflow: str, payload: AiWorkflowIn, context: dict[str, Any]) -> Any:
    ...
    if workflow == "generate_emotion_seed":
        ...
        source = emotional_source_text(payload, context)
        return {
            "core_emotion": "想靠近真相，却害怕真相证明自己曾经亲手放弃过重要之人。",
            "surface_action": f"{protagonist}继续追查线索，表面保持冷静。",
            "suppressed_need": "希望有人确认她没有把所有错误都记错。",
            "sensory_anchor": "雨声、旧纸气味、门禁冷光",
            "body_signal": "指尖停在证词边缘，没有立刻翻页。",
            "next_echo": "后文每次出现雨声，都提醒这份未被承认的亏欠。",
            "source_excerpt": trim_text(source, 240),
        }
    if workflow == "emotion_archaeology":
        source = emotional_source_text(payload, context)
        return {
            "surface_layer": "人物在推进调查或对话，行为目标清晰。",
            "emotional_layer": "真正的情绪是迟来的愧疚、戒备和想被理解的冲动。",
            "intention_layer": "她想控制局面，避免再次被别人替她解释过去。",
            "subconscious_layer": "她害怕自己并不是受害者，而是某个灾难的参与者。",
            "resonance_layer": "读者应感到：角色越冷静，内里越接近崩裂。",
            "rewrite_hint": "少写“她很痛苦”，多写停顿、避开目光、触碰旧物时的身体反应。",
            "source_excerpt": trim_text(source, 240),
        }
    if workflow == "dialogue_subtext_excavation":
        return {
            "spoken_line": "你确定要进去？",
            "subtext": "我知道里面有什么，也知道你进去以后就不能再装作不知道。",
            "hidden_counterline": "如果答案已经在里面等我，我不进去，它也会出来找我。",
            "rewrite": "“你确定？”他问。\n她看了他一眼，没有回答确定不确定，只把那张缺角的卡片攥进掌心。",
            "avoid": "不要让角色直接解释“我害怕但必须面对”。",
        }
    if workflow == "analyze_reader_pull":
        source = emotional_source_text(payload, context)
        return {
            "hook_strength": 78,
            "emotional_debt": "角色知道一部分真相却不愿承认，读者会追问她到底隐瞒了什么。",
            "open_question": "那份证词为什么有她自己的签名？",
            "payoff_need": "后续 1-3 章内必须让签名造成现实后果，而不是只作为气氛道具。",
            "risk": "如果下一段只继续解释设定，追读力会下降。",
            "source_excerpt": trim_text(source, 240),
        }
...
def build_stub_ai_output(
    ...
    titles = {
        ...
        "emotion_archaeology": "五层情感考古",
        "dialogue_subtext_excavation": "对话潜台词",
        "analyze_reader_pull": "追读力检查",
        "deepen_and_bury": "情感加深藏回",
        "anti_ai_polish": "去 AI 味",
    ...
    elif workflow == "deepen_and_bury":
        text = build_emotional_rewrite(payload, context)
    elif workflow == "anti_ai_polish":
        text = build_emotional_rewrite(payload, context, anti_ai=True)
...
def system_prompt_for_workflow(workflow: str) -> str:
    if workflow in {"generate_chapter_draft", "revise_selection", "deepen_and_bury", "anti_ai_polish"}:
        return (
            ...
            "情感增强时要优先使用动作、停顿、感官物件、对白缺口和回避反应呈现情绪，不要把潜台词直接说破。"
            "deepen_and_bury 只做局部加深和藏回，不要重写成提纲；anti_ai_polish 要去除模板化总结、口号式心理描写和解释腔。"
            "如果用户输入包含章节编号，以 chapter_number 为准，绝不要把 chapter_id 当成章节序号。"
        )
    if workflow in {"generate_emotion_seed", "emotion_archaeology", "dialogue_subtext_excavation", "analyze_reader_pull"}:
        return (
