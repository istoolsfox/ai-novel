...
def test_emotional_enrichment_workflows_return_actionable_outputs(monkeypatch, tmp_path):
    client = make_client(monkeypatch, tmp_path)
    project = client.post("/api/projects", json={"title": "情感增强测试"}).json()
    chapter = client.post(
        f"/api/projects/{project['id']}/chapters",
        json={
            "chapter_number": 1,
    ...
    seed = client.post(
        ...
        json={"chapter_id": chapter["id"], "content": chapter["draft"]},
    )
    assert seed.status_code == 200
    seed_payload = seed.json()
    assert seed_payload["structured"]["core_emotion"]
    assert seed_payload["structured"]["sensory_anchor"]

    deepened = client.post(
        f"/api/projects/{project['id']}/ai/deepen_and_bury",
        json={"chapter_id": chapter["id"], "selected_text": "她没有立刻说话。"},
    )
    assert deepened.status_code == 200
    deepened_text = deepened.json()["text"]
    assert "指尖" in deepened_text or "雨水" in deepened_text
    assert "情感建议" not in deepened_text
