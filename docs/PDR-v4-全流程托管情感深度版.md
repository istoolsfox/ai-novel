...
- ❌ 不引入向量数据库（MVP 用卷记忆 + 全文检索够）
- ❌ 不引入 Celery/Redis（后台线程够）
- ❌ ~~不重写前端为 Vue（保留 React）~~ → **已决策：切换到 Vue 3 + Naive UI**（2026-07-02 用户确认）
- ❌ 不破坏 v2 单章手动模式（保留为 fallback）
- ❌ 不要求 Python 3.14（3.13 够）
- ❌ 不做硬约束式情感规划（v2 已证明是精致程式化）
- ❌ 不做"做加法"的修写（deepen 做减法优先）
- ❌ 不把追读力做成纯爽点（与情感深度联动）
- ❌ 不把情感气候图做成硬约束（气候非天气）
